 ...
 *
 * V3:
 * - freeze metadata "hard": blocca anche setData/setDataBatch sulla key LSP4 metadata
 */
contract First10Purchases2026_V3 is LSP7DigitalAsset {
  ...
  // ------------------------------------------------------------
  // HARD FREEZE: impedisce bypass via ERC725Y setData/setDataBatch
  // ------------------------------------------------------------
  ...
  function setData(bytes32 key, bytes memory value)
    ...
    onlyOwner
  {
    if (metadataFrozen && key == _LSP4_METADATA_KEY) revert MetadataIsFrozen();
    super.setData(key, value);
  }

  function setDataBatch(bytes32[] memory keys, bytes[] memory values)
    public
    payable
    virtual
    override
    onlyOwner
  {
    if (metadataFrozen) {
      for (uint256 i = 0; i < keys.length; i++) {
        if (keys[i] == _LSP4_METADATA_KEY) revert MetadataIsFrozen();
      }
    }
